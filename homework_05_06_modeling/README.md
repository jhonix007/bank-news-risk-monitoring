...
В текущей clean-версии основной финальной моделью фиксируется `Tuned combined TF-IDF`. Она не зависит от внешнего LLM-teacher ни при обучении, ни на этапе inference. LLM-distillation варианты с `GPT-4.1` и `GPT-5.5` показаны как advanced/reference comparison и не заменяют основную финальную модель задним числом.
...
Дополнительно `Tuned combined TF-IDF` показывает самый высокий PR-AUC на test. Это означает, что модель хорошо ранжирует риск-сигналы по вероятности и остаётся сильным решением для сценария, где threshold может подбираться под допустимую нагрузку аналитиков.
...
- LLM-distillation результаты интерпретируются как diagnostic comparison, а не как замена основной финальной classical ML модели.
- для LLM-distillation teacher нужен на этапе подготовки train soft labels, но не нужен на inference после обучения student-модели.
