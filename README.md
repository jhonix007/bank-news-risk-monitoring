...
Учебный MVP-сервис для выявления риск-сигналов в банковских новостях.
...
MVP автоматизирует первичную сортировку новостей и помогает получить таблицу с `risk_score` и `alert_flag`.
...
Платежи учебные и симулированные.
...
Если `.joblib`-артефакт отсутствует, используется fallback-модель `fallback_keyword_model`. Она нужна только для демонстрационного запуска MVP и не является финальной исследовательской моделью.
...
Исследовательские ноутбуки, домашние задания и эксперименты вынесены в папку `research/`, чтобы не смешивать сервисную часть MVP с этапами подготовки данных и моделирования.
...
Сервисная часть MVP использует финальную модельную логику `TF-IDF student v2b`; Qwen/RuBERT-ноутбуки сохранены как исследовательские эксперименты и не запускаются при старте Docker-сервиса.

## Compatibility links

Для сохранения ранее отправленных преподавателю ссылок notebook `07_postprocessing_and_error_analysis.ipynb` продублирован по старому пути:

`homework_07_postprocessing/notebooks/07_postprocessing_and_error_analysis.ipynb`

Основная исследовательская версия также хранится в `research/homework_07_postprocessing/notebooks/`.
...
Интерфейс MVP выполнен в сдержанной финансовой стилистике: светлый фон, белые карточки, графитовый текст и бордовый акцентный цвет. В hero-блоке главной страницы используется деликатный SVG-фон с тонкой сеткой и абстрактной data-line графикой. Оформление вдохновлено корпоративными интерфейсами банковского и регуляторного сектора, не использует официальную символику Банка России и не позиционирует сервис как официальный продукт регулятора.
...
* Это учебный MVP.
* Платежи и подписка симулируются.
* Промышленный сбор новостей не реализован.
* Пользователь загружает обычный CSV со списком новостей.
* Сервис не делает live scraping.
* Сервис не настраивает источники мониторинга.
* Сервис не требует выбора банков в интерфейсе.
* `entity_norm` можно передать во входном файле, но это необязательно.
* Финальная исследовательская модель — `TF-IDF student v2b`.
* Если `.joblib`-артефакт отсутствует, используется fallback-модель для демонстрации упаковки сервиса.
* Нейросетевые эксперименты с `rubert-tiny2` и `Qwen2.5-0.5B + LoRA` не выбраны как финальные из-за более слабих метрик на текущем размере выборки.
